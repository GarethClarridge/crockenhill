...
- **Worth-it gate (autonomous PRs only):** every unattended autonomous PR — including Jules,
  Codex, and Gemini — must state **Who benefits** and **What observably improves**
...
- Correct use of `Image::read()` (not the v2-era `Image::make()`) — `intervention/image-laravel` is v4
