...
<form class="" action="/church/members/meetings/{{ $meeting->slug }}" method="post">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
      ...
      <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
             id="slug" name="slug" type="text" value="{{ old('slug', $meeting->slug) }}" required>
      ...
      <select class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
        ...
        @foreach(\App\Enums\MeetingType::cases() as $type)
          <option value="{{ $type->value }}" {{ old('type', $meeting->type?->value) === $type->value ? 'selected' : '' }}>
            {{ $type->label() }}
          </option>
        @endforeach
      </select>
      ...
      <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
             id="day" name="day" type="text" value="{{ old('day', $meeting->day) }}" placeholder="e.g., Wednesday">

      <label class="block mt-6" for="location">Location</label>
      <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
             id="location" name="location" type="text" value="{{ old('location', $meeting->location) }}" placeholder="e.g., Church Hall">
      ...
      <textarea class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                id="who" name="who" rows="3" placeholder="e.g., All adults welcome">{{ old('who', $meeting->who) }}</textarea>
    </div>
    ...
    <div>
      ...
      <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
             id="meeting_date" name="meeting_date" type="date" value="{{ old('meeting_date', $meeting->meeting_date?->format('Y-m-d')) }}">
      ...
      <div class="grid grid-cols-2 gap-3 pt-2">
        <div class="h-full flex items-center pl-4 bg-gray-300 rounded">
          <input id="pictures-yes" type="radio" value="1" name="pictures" 
                 ...
                 {{ old('pictures', $meeting->pictures ? '1' : '0') === '1' ? 'checked' : '' }}>
        ...
        <div class="h-full flex items-center pl-4 bg-gray-300 rounded">
          <input id="pictures-no" type="radio" value="0" name="pictures" 
                 class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-400 focus:ring-blue-500 focus:ring-2"
                 {{ old('pictures', $meeting->pictures ? '1' : '0') === '0' ? 'checked' : '' }}>
      ...
      <div class="mt-6">
        <label class="flex items-center">
          <input type="checkbox" name="is_recurring" value="1" id="is_recurring" 
                 ...
                 {{ old('is_recurring', $meeting->is_recurring) ? 'checked' : '' }}>
      ...
      <div id="frequency-section" class="mt-6" style="display: none;">
        ...
        <select class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
          ...
          @foreach(\App\Enums\MeetingFrequency::cases() as $freq)
            <option value="{{ $freq->value }}" {{ old('frequency', $meeting->frequency?->value) === $freq->value ? 'selected' : '' }}>
              {{ $freq->label() }}
            </option>
          @endforeach
        </select>
  ...
  <div class="form-actions mt-8">
    <div class="grid gap-2 mb-3">
      <x-form-button variant="primary" size="xl">Update Meeting</x-form-button>
    </div>
...
// Show frequency section if it was checked on page load (after validation error or existing data)
