...
<th {{ $attributes->merge(['class' => 'px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider']) }}
    @if($sortable) aria-sort="{{ $sortBy === $key ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}" @endif>
    @if($sortable)
        <button wire:click="sort('{{ $key }}')"
                ...
                @if($sortBy === $key) aria-label="{{ $label }}: sorted {{ $sortDirection === 'asc' ? 'ascending' : 'descending' }}" @else aria-label="{{ $label }}: click to sort" @endif>
            <span>{{ $label }}</span>
