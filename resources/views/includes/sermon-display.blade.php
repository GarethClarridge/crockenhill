<div class="card p-0">
  <div class="row g-0">
    @if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/images/sermons/' . $sermon->series . '.png'))
      <div class="sermon-icon col-md-6 ratio ratio-1x1">
        <img src="/images/sermons/{{$sermon->series}}.jpg" alt="{{$sermon->series}}">
      </div>
    @else
      <div class="sermon-icon col-md-6 d-flex align-items-center justify-content-center ratio ratio-1x1" style="background-image: url('/images/sermons/standard.jpg'); background-size:100% auto;">
        <span class="h1 text-white mx-auto align-middle text-center d-flex align-items-center justify-content-center px-5">
          {{$sermon->series}}
        </span>
      </div>
    @endif
        ...
        <ul class="list-group list-group-flush">
          @if (($sermon->title != null))
            <li class="list-group-item">
              <h3>
                <a class="my-auto" href="/church/sermons/{{date('Y', strtotime($sermon->date))}}/{{date('m', strtotime($sermon->date))}}/{{$sermon->slug}}">
                  {{$sermon->title}}
                </a>
              </h3>
            </li>
