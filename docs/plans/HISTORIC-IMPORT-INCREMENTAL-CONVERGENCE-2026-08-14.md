...
6. **Calibration and model comparison in progress 2026-08-27.** The incumbent pipeline inputs are
   ...
   **Defect 2 — recovery was poisoned by its own prompt.** The retry inherited
   ...
   `unobservable` to `recovered`. **Open question, deliberately not actioned:** if whole-service
   priming induces these loops, it may be *causing* artefacts corpus-wide rather than merely failing
   to clear them. That bears on all 470 identities and should be measured before the definitive run;
...
| **Measure whether whole-service priming causes transcript loops corpus-wide** | Coding agent, before definitive dispatch. IC5 item 6 established that `transcription.prompts.full_service` makes the model emit service-shaped text over non-speech: the same clip returns a real hymn unprimed and "The Holy Spirit is a great song for the Holy Spirit" ×8 primed. Recovery no longer inherits it, but the *production* prompt is unchanged, so the same mechanism may be generating these artefacts across the corpus rather than only defeating their repair. Replay the detector over the definitive run's transcripts, or probe a sample unprimed, before accepting the run's completeness | Definitive video dispatch |
