...
|---|---|---|---|
| 1 | **Settle Email + OpenLP RG-A** | Portable-subset apply behaviour proved **DONE 2026-08-24** (`cfe190069`): `oos:import-archive --apply-bundle` now exits 0 and imports every independently valid entry while leaving structurally invalid/held entries `Pending` for review, instead of failing the whole batch (full suite 7,166 green, PHPStan clean). Email manifest v3 has 557 entries (554 include, 3 exclude), 540 primary identities. **Accepted-holds draft reviewed and approved by the operator 2026-08-24** (all 18 entries as written); `oos:generate-corpus-expectation --accepted-holds=` re-run against `oos-curation-manifest.json` — 554 approved sources, 540 identities, 18 accepted holds folded in, written to `storage/scratch/oos-corpus-expectation-20260824.json` (gitignored scratch artifact, not committed). OpenLP is frozen at 655 entries (614 include, 36 duplicate-of, 5 exclude) and has its expectation producer. Remaining before this slice closes: a coding agent runs the combined clean Email+OpenLP RG-A and the operator rules on any genuinely-irreducible high-volume proposal classes it surfaces. | A clean rehearsal stages the current Email portable-valid subset and all 614 OpenLP includes; every invalid/held Email entry is named; Email and OpenLP expectations plus one combined membership artifact reconcile; the census has no corpus blocker. Review only high-volume proposal classes with a plausible shared fix. Enumerated residual proposals do **not** block video merely because no class-review row was created for every unique title. |
