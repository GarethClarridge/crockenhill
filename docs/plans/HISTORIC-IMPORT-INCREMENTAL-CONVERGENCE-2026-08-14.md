...
| **Build a producer for `crockenhill.historic-runtime.v1` runtime evidence** | Coding agent. Nothing in the repo writes this artifact — a repo-wide search for the format string returns only `HistoricImportRuntimePreflight` and its unit test, the same "validator without a producer" shape as the F66 acquisition gate. The 15-key contract needs genuinely observed values: pinned `image_digest`, SHA-256 of the in-container FFmpeg/ffprobe binaries, per-stage non-mock provider with `credential_present` **and** `connectivity_verified`, storage encryption at rest and in transit, plus `schema`, `database`, `queues`, `resources`, `clock`, `outbound_probe`, `prompts`, `algorithms`. Hand-authoring it would manufacture evidence, so the producer must observe, not assert | Any **production** historic import operation, and therefore the definitive dispatch and release evidence. Explicitly **not** the rehearsal calibration — see the narrowing below |
