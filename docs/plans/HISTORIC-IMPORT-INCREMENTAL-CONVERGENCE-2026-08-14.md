...
> to 20 the same day — `2024-10-06` and `2016-05-01-pm` — and `oos:generate-corpus-expectation` has
...
|---|---|---|
| Re-run the combined clean-rehearsal RG-A against the current expectation and inspect only repeated/high-volume proposal classes it surfaces. **Regenerate the expectation first**: `2024-10-06` was fixed 2026-08-24 (resolver `tomorrow` precedence, see the journal table below) and must come *off* the accepted-holds draft, taking it **20 → 19**. Re-run `oos:generate-corpus-expectation --accepted-holds=` and read the new triple off that run rather than adjusting the old `554/540/20` by hand — the identity count moves too, and these three figures are not complementary (540 + 20 > 554), so it cannot be inferred | Coding agent (rehearsal), then operator rules on genuinely irreducible repeated classes | REV-D4 video trigger |
| **`low_confidence` is a constant, not a signal — decide whether to fix the flag or the config.** `CompileOosSemanticAnnotations.php:350` assigns a flat `'confidence' => 0.75` to every plan it compiles (the semantic parser computes no per-plan score), and `ImportOosArchiveCommand.php:889-892` raises `low_confidence` whenever confidence < `auto_import_threshold` = **0.90**. 0.75 < 0.90 always, so **all 710 dated plans in the rehearsal carry `low_confidence`, 100%, by construction** — 697/697 Sunday and 13/13 non-Sunday. Two consequences: `auto_import_threshold` is dead config (nothing can ever auto-import under the semantic parser), and every hold-reason set is padded with a term that discriminates nothing, which matters directly for the operator hold review above. ~~Looks like a cutover fossil: the legacy parser produced a real score so the 0.90 gate discriminated; Delivery 7 replaced scoring with a constant and left the threshold.~~ **NOT a fossil — corrected 2026-08-24 after reading the gate.** The arrangement is deliberate and, critically, **`low_confidence` is load-bearing: do NOT simply stop emitting it.** `OosSemanticCorrectnessScorer`'s routing docblock states outright that "the semantic compiler fixes extracted confidence at 0.75, below the 0.90 threshold", making `AutoImportable` unreachable *by design*; the live path is the REV-D2 evidence tier, `OosEmailServicePlan::isEvidenceImportable()`, which requires `disposition === ReviewRequired` **and `holdReasons !== []`**. That non-empty test exists to exclude legacy pre-validator parses, and under the semantic parser the constant `LowConfidence` is what satisfies it. Measured on the rehearsal: **666 of 674 `review_required` plans carry `low_confidence` as their ONLY hold reason** (8 have others, 0 have none). Removing the constant would empty `holdReasons` for those 666 and drop them straight out of the evidence tier — a silent completeness regression in the worst direction. **An earlier draft of this row recommended exactly that; it was wrong.** The genuinely honest fix is therefore larger than it looks: make the "this parse was validated by the semantic validator" predicate *explicit* (a real field on the plan) so `isEvidenceImportable()` stops inferring validated-ness from a non-empty hold list, and only then retire the flat `0.75` and the constant flag. That touches the DTO, `OosArchiveIdentityResolver`, `OosEmailParserService`, `ChurchServiceReviewSynchronizer`, the scorer and their tests, and it changes unattended import gating for 666 plans — so it needs its own slice with a full-suite plus zero-spend replay check proving the same 666 stay evidence-importable. **It does not, however, invalidate a pending RG-A run** (an earlier draft of this row implied it did, wrongly): the membership certificate binds to `membership_hash`, `source_key_hash` and `batch_hash` — data identity, not code identity, and it records no application commit or parser-surface hash — and `parser_surface_commit` invalidates no cache, so touching surface files forces no re-annotation. The only real sequencing point is attribution: the refactor is behaviour-preserving by intent, so if RG-A's numbers move it is a bug in the refactor, and that reads more cleanly as its own signal than mixed into an RG-A. That argues mildly for doing it *before* RG-A, so the certificate describes the final import gate. Until it lands, `auto_import_threshold` is best documented as deliberately unreachable rather than silently repaired | Operator rules on whether to schedule the explicit-predicate refactor; nobody removes `low_confidence` before it lands | Hold-reason review readability only; **not** a corpus blocker, and not urgent |
