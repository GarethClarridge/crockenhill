...
> authorisation of the slice 5 operation/round for the definitive 470-identity video dispatch (the
> worksheet exceptions and manifest freeze that used to sit here are done); recovery of authoritative
> source material for any current-era services still reported as unevidenced; every release batch; and
> the §8.4 publication-policy decision.
...
| 4 | **Curate and calibrate video** | **DONE 2026-08-28.** Curation froze 2026-08-26; calibration inputs banked and transcript pathology closed 2026-08-27; human truth review, blinded scoring, both 6c/6d gates, the operator adoption decision and its deploy all completed 2026-08-28 (see item 6d). A transcript-pathology guard, two defects found in it (an outage that destroyed cues, and recovery inheriting the whole-service prompt that made it hallucinate on music), and artifact-level regeneration of four flagged identities all landed 2026-08-27 — see item 6 for the measurements. The completed sermon-analysis arms are unaffected and must not be re-run; the structure inputs were rebuilt as `structure-eval-20260827/manifest.json`. The adjudicated worksheet contains 472 identities / 510 recordings: 470 includes and 2 excludes. Coverage is 290 `full`, 172 `short_partial` and 10 `fragmented` (8 included). Frozen batch `historic-video-full-corpus-20260826`: private manifest `historic-video-curation-manifest-20260826.json`, manifest hash `1ae7e4fc9da6f0cb3db10f167eea86d1d89914a36837a92e23bc05c32ac85298`, derived plan hash `8ecec582659ea18d796ffa50adffeef20cebe1ca529d8bf233c177b9746d29fe`. The bounded sermon-analysis evaluator, banked-input hashes, six review worksheets and structure-manifest skeleton exist. Successful first draws: incumbent Terra/low, 6/6, $0.090694 actual and $7.104363 projected for 470; candidate Luna/low, 6/6, $0.0090454 actual and $0.708556 projected. Both were subsequently scored: `gpt-5.6-luna`/low cleared its draw-1 safety gate but its draw 2 carried a genuine automatic failure (2024-12-22-evening, wrong Scripture reference), adopted anyway on the operator's recorded corpus-composition annotation; structure arms 09-14 cleared 6c and 6d for both `gpt-5.6-terra`/medium and `gpt-5.6-luna`/medium, and the cheapest passing arm was adopted. **Superseded 2026-09-01:** re-frozen as `historic-video-curation-manifest-20260901.json` (`d25d2085…`/`9351fa4e…`), 474 identities / 464 includes / 10 excludes, after seven drive replacements and two reversed exclusions — see §3. The isolated codec-mismatch fallback was proved from two temporary clips derived from the untouched `2024-12-22-evening` source (H.264/AAC versus VP9/Opus): `historic-video-reencode-proof-20260827.json` records `concat_reencoded`, input/output hashes and zero processing state. | Closed. The six human truth worksheets and blinded scoring are complete; the item 6c/6d gates were applied to both call families and the adoption decision is recorded and deployed under item 6d. Definitive dispatch remains separately authorised and is now the only thing standing between here and slice 5. Not carried out and no longer blocking: p50/p95 wall time, retries/rechecks and accepted worker width were not measured on the calibration arms - they are throughput properties of the definitive run and are recorded there instead. |
| 5 | **Run definitive historic video processing and re-census** | **NEXT — blocked only on operator authorisation (2026-08-28).** Slice 4 calibration evidence is complete, the full-corpus manifest is frozen, and both adopted models (`gpt-5.6-luna` for structure and sermon analysis) are live in `.env` with the queue workers restarted. What remains is the separately approved operation/round required by §7, which is a human decision, not engineering work. | Dispatch the same frozen manifest in bounded resumable passes. Then regenerate membership and the proposal census. The 223-service awaiting-evidence population must either finalise on a `full` recording or remain explicitly unresolved; re-evaluate the 135 prior disagreements with the new evidence before calling any residue irreducible. The final convergence gate remains red until every surviving class has a truthful terminal disposition. |
...
**Safety qualification discovered 2026-08-25; IMPLEMENTED 2026-08-25 — see §0.1 slice 3.**
...
3. **Video worksheet → exception review → freeze done 2026-08-26.** The adjudicated worksheet is
   ...
   manifest hash is `1ae7e4fc9da6f0cb3db10f167eea86d1d89914a36837a92e23bc05c32ac85298` and
   the derived plan hash is `8ecec582659ea18d796ffa50adffeef20cebe1ca529d8bf233c177b9746d29fe`.
   **Hash provenance correction (2026-08-26):** the private manifest was created at 13:11:28 BST,
   while its derived plan report was written at 20:04:19 BST. The previous `7b7bf688…` and
   `65dfd15c…` values were first entered into this document by commit `12dc3fd3a` at 19:13:44 BST;
   they do not match the frozen manifest or its derived plan. `PrivateEvidenceFile::writeOnce`
   prevents an in-place manifest rewrite, and the later verification optimisation changed only
   source-content re-reading, not the hash inputs. This is a documentation/transcription correction;
   the private manifest was not re-frozen.
   ...
   **Run bounded/calibration passes without it; run the definitive pass with it.** Its unique
   ...
   the frozen `1ae7e4fc…`/`8ecec582…` artifacts need no re-freeze or re-approval. What a run without
   the flag gives up is only the stronger claim that holding a plan hash proves the whole corpus was
   intact at plan time.

   **Correction, and the flag's removal (2026-09-01).** The justification above is wrong on its
   central claim. `HistoricVideoImporter` does *not* re-check SHA-256 before FFmpeg, and no tamper
   test proves that it does — `assertApprovedSourceMetadata()` validates that the approved `sha256`
   is well-formed 64-hex and then compares **byte size only**, and the importer records
   `sha256_basis` as `approved_manifest_not_reverified_at_dispatch`. Nothing anywhere re-verified a
   frozen curation hash: `VerifyHistoricSourceAcquisitionCommand` and
   `AuditHistoricImportAssetsCommand` do not read them, and the sole `verifySourceContents: true`
   caller sat behind `PrivateEvidenceFile::writeOnce()`, where it could re-read nothing but files
   hashed seconds earlier — making `historic-import:capture-video-curation` read the ~1.0 TB corpus
   **twice**, about six hours, for no guarantee. `$verifySourceContents` and the `hash_file()`
   comparison in `verifiedPath()` are therefore deleted, with the test that asserted the branch.
   The unconditional stat checks are unchanged and are what actually enforce the manifest. The
   SHA-256 **field** stays in schema v4: removing it is a v5 migration that would make the frozen
   manifests unparseable — orphaning the `manifest_hash` recorded on completed runs — and
   `fileContentIdentity()` becomes load-bearing the moment a `duplicate_of` is declared, where
   `byte_size` alone degrades "byte-identical target" to "same file size".

   **Re-freeze 2026-09-01.** Seven morning identities were replaced or augmented from the church PC,
   so the manifest was re-cut as `storage/app/private/historic-video-curation-manifest-20260901.json`
   — manifest hash `d25d20858106404d07d9b3fe1c73e5099fa57017bb138339ac6d127aba346e20`, derived plan
   hash `9351fa4e5e8607f7a728d52fd3c31cb7370d443dda9959dcc38871ea59818bfd`, batch key unchanged
   because it feeds `HistoricSourceCustodyCapture` and `HistoricItemGroundTruth`. It contains 474
   identities / 517 declared recordings: 464 includes and 10 excludes; 292 `full`, 163
   `short_partial`, 9 `fragmented`. Capture was carry-forward — reuse the frozen hash wherever
   `(relative_path, byte_size)` is unchanged, hash only what is new — 504 reused, 13 computed,
   2.29 GB in 19.5 seconds, validated by running the real `plan()` against the drive.

   Two exclusions were **reversed**, because the replacements falsify their recorded reasons:
   `2023-07-16-morning`, whose reason stated the sermon "is in no file held on the drive" and whose
   new single recording is that 67.49-minute service; and `2020-06-28-morning`, where a real
   23.18-minute sermon now sits beside the two 49-second incidental clips. Grade coupling constrains
   the rest: `corroboration === fragmented` iff more than one file, so the three identities that
   *gained* segments (2025-05-18, 2025-08-10, 2026-03-15 mornings) are `fragmented` despite now
   running 25-54 minutes, and only the two that became single whole-service files reach `full`.
   Two new excluded siblings keep superseded files declared without dispatching them
   (`2020-06-14-morning-superseded-copy`, `2020-06-28-morning-incidental-clips`); an excluded entry
   may share a date and service with an included one because `validateServiceIdentities()` skips
   non-includes. `2024-07-26-morning` was found moved out of the corpus root and carries its
   historic hash forward, since `capture()` hashes every entry regardless of disposition.

   **Not yet done:** no operation is prepared for `9351fa4e…`. The 28 runs completed under
   `1ae7e4fc…` will report `skip-exists` rather than `resume-completed`, because the resume
   `dedup_key` is `sha256("historic-video\0{manifestHash}\0{item_key}")`. `2024-07-28-morning` is
   declared with its better 64.47-minute recording but stays blocked by the existing completed run
   and sermon 890, on the operator's recorded decision to leave that sermon in place.
3a. **Runtime evidence is now required only on a production target (2026-08-26).**
   `historic-import:prepare-operation` demanded `--runtime-evidence` unconditionally, so no
   operation could be created at all — and therefore no non-dry-run dispatch, calibration included.
   The evidence contract was built for the one-shot GO in commit `ceaadb103` (2026-08-09), five days
   before REV-D1–D4 replaced that model with incremental convergence; this plan never mentions
   runtime evidence anywhere, and no producer for the artifact exists.

   Three findings decided the narrowing. `HistoricImportOperationIdentity::fromBindings()` already
   accepts a null runtime fingerprint and falls back to the target fingerprint, so the absent case
   is modelled and tested rather than novel. The only consumer of the stored value,
   `HistoricImportCheckpointRuntime::assertRuntimeBinding()`, is a bare `hash_equals` — it detects a
   runtime that changed mid-operation and never re-validates the attestation's substance. And that
   consumer is called from tests only; no dispatch path admits checkpoints.

   So a rehearsal target may now omit the flag, keeping the change-detection property and giving up
   only the production-grade claim it has no business making. A production target still requires it,
   and `guardsCurrentEnvironment()` fails closed — an unreadable or misconfigured anchor counts as
...
6. **Calibration and model comparison in progress 2026-08-27.** The incumbent pipeline inputs are
   banked and the first incumbent Terra/low and candidate Luna/low sermon-analysis draws completed
   successfully; neither is accepted until blinded human scoring applies item 6c. The bounded
   evaluator added for this purpose is `sermons:evaluate-analysis`, with its IC8 deletion trigger
   recorded in the command docblock. The structure runner remains `structure:evaluate` and its
   six-item manifest is still a review-pack skeleton pending human truth completion. Use these
   ...
   re-encoding: all included fragmented identities are lossless-compatible. **Fallback proof done
   2026-08-27.** `historic-import:prove-video-reencode-fallback` ran the same extracted FFmpeg
   concatenator used by the importer over two two-second clips derived from the untouched
   `2024-12-22-evening` recording, one H.264/AAC and one intentionally incompatible VP9/Opus
   derivative. The private report `historic-video-reencode-proof-20260827.json` records distinct
   input fingerprints, the re-encoded H.264/AAC output hash, `result: concat_reencoded` and
   `processing_state_created: false`; the temporary output was deleted and neither derivative was
   admitted to a manifest or processing log. Focused coverage regenerates incompatible clips and
   proves the no-mutation contract.

   **Transcript-pathology guard added 2026-08-27; two defects found in it and fixed the same day.**
   Full-service transcription now detects a sustained run of repeated low-information cues (>=6
   identical consecutive cues spanning >=120s, phrase <=12 words, gap <=60s), extracts and
   retranscribes only that audio window, and replaces the suspect cues when the retry is non-empty
   and passes the same detector. A still-pathological retry removes the corrupt cues and persists a
   timestamped `TRANSCRIPT UNOBSERVABLE` window in the transcript artifact and processing metadata.
   That marker reaches structure prompts, portable processing metadata and Livestream source
   provenance; any historic recording carrying one is neutral for song membership/count/order even
   when its manifest grade is `full`.

   The `max_phrase_words` widening from 8 to 12 was **measured, not assumed**: replayed across all
   nine banked service transcripts it adds exactly two windows — `2024-01-14-morning` 09:34-14:02
   (the reported 11-word artefact) and `2025-02-02-morning` 01:16:40-01:18:58 — and both recover
   cleanly, so the widening carries no false-positive cost on the corpus where it can be checked.
   Four identities carry flagged windows in total, the other two being `2024-12-22-evening`
   00:00-19:59 (40 `Thank you.` cues over its carols) and `2026-07-26-morning` 19:57-22:29.

   **Defect 1 — an outage destroyed transcript content.** One `catch (Throwable)` spanned both the
   attempt and the verdict, so a missing FFmpeg or a transcription timeout deleted real cues *and*,
   through the new projector gate, disqualified the whole recording from corroborating songs.
   Proved red (41 cues reduced to 1). The attempt is now isolated in a method returning null, and
   the two outcomes are distinct: `retranscription_unavailable` keeps every original cue and only
   flags the window, because nothing was learned about the audio; `retranscription_failed` still
   deletes, because the audio was examined and yields nothing.

   **Defect 2 — recovery was poisoned by its own prompt.** The retry inherited
   `transcription.prompts.full_service` ("welcome, hymns and songs, prayers, Bible readings, notices
   and a sermon"), which primes the model to emit service-shaped text over non-speech — so recovery
   hallucinated on precisely the music-only windows it exists to rescue. Measured on the same clip,
   same model: unprimed it returns the real hymn ("All praise to Him, who's loved and seen"); primed
   it returns "The Holy Spirit is a great song for the Holy Spirit" eight times. `transcribeService`
   now takes an optional `?string $prompt` (null keeps the configured prompt, `''` asks for none)
   across all three implementations, and recovery passes `''`. This moved `2026-07-26-morning` from
   `unobservable` to `recovered`. **That open question is now measured and closed 2026-08-28** (see the
   §10 row): whole-service priming does roughly double looping, but removing it flags *more* material
   unobservable, not less, and the unprimed arm loops in equally service-shaped language. The
   production prompt stays as it is; what the measurement actually surfaced is that catastrophic
   collapse occurs in both arms and does not reproduce between passes.

   **Regeneration done 2026-08-27, without reprocessing.** A rehearsal-only
   `historic-import:recover-calibration-transcripts` runs the same `ServiceTranscriptRecovery` over
   banked audio and banked transcripts read straight off the drive. It was built after establishing
   that an importer seam is unnecessary: the recovery service touches no database, so no new
   processing records are minted and the completed-item guard is not bypassed. A partially built
   `--reprocess-completed` importer flag was removed as orphaned; it also over-promised, claiming
   fresh processing identities while only setting `$force = true`. Report:
   `calibration-recovery-20260827b/report.json` (banked inputs verified unchanged by hash;
   `processing_records_created: false`). Outcomes: `2025-02-02-morning` recovered to the full
   correct lyrics of *What a Friend We Have in Jesus*; `2026-07-26-morning` recovered, though
   partially — four looped cues survive below the six-cue floor, but the sermon transition at
   1343.1s is now captured; `2024-01-14-morning` and `2024-12-22-evening` remain `unobservable`, the
   latter reproducing its artefact exactly on isolated retry, which is evidence the audio itself is
   the limit rather than the transcription.

   **The completed sermon-analysis arms are unaffected and must not be re-run.** Only
   `2026-07-26-morning`'s window overlaps its sermon span, and the sentence recovery added is absent
   from `sermon_871.md`; all six banked sermon transcripts are byte-unchanged and still hash-match
   `input_sha256` in both arm reports. The consequence falls on structure instead: that recording's
   sermon start now has a candidate at 1343.1s against the previous 1377.6s, a 34-second move that
   sits outside item 6c's 30-second boundary tolerance, so it must be settled by human truth rather
   than inherited. The structure inputs were rebuilt accordingly as
   `calibration-review-pack/structure-eval-20260827/manifest.json`, with each identity's effective
   transcript colocated and referenced relatively — the skeleton's absolute `/Volumes` paths never
   resolved inside the container the runner executes in — plus a v2 `banked-input-hashes.json`
   recording each transcript's origin, recovery disposition and superseded hash. Expectations remain
   empty for all six pending item 6a. Residual, below the detector floor and therefore uncleared:
   `sermon_872.md` opens with a stray `"Thank you."`.

   Use the project's configured local Whisper paths for both whole-
   ...
   **6d operator decision, recorded 2026-08-28.** Both call families cleared 6c and 6d. Structure
   detection produced two passing candidates (`gpt-5.6-terra`/medium, 52.3% cheaper; `gpt-5.6-luna`/medium,
   94.5% cheaper) — the operator adopts the cheapest passing arm per the rule above.

   | call family | adopted | incumbent retired | evidence |
   |---|---|---|---|
   | Service structure | `gpt-5.6-luna` / `medium` | `gpt-5.6-sol` / `medium` | `storage/app/private/structure-luna-ruling-20260828.md` |
   | Sermon analysis | `gpt-5.6-luna` / `low` | `gpt-5.6-terra` / `low` | `storage/app/private/calibration-review-pack/sermon-analysis-unblinded-ruling-20260828.md` + `sermon-analysis-round2-ruling-20260828.md` |

   Sermon analysis adopted with the operator's explicit annotation on the round-2 finding: the one
   draw-2 automatic failure (2024-12-22-evening, wrong Scripture reference) is corpus-composition-driven
   — the calibration corpus's one carol-format identity out of six lacks the reading-immediately-precedes-sermon
   structure the model otherwise relies on correctly — not treated as disqualifying. `gpt-5.6-luna`/none
   was evaluated for sermon analysis and cleared its own single-draw gate, but was not pursued
   further: its cost is not significantly lower than `gpt-5.6-luna`/low (1.0% apart, both ~90% below
   the incumbent), so the marginal saving didn't justify a further paid draw.

   The decision does not itself change `config(...)` defaults or the
   `SERVICE_STRUCTURE_MODEL`/`ANALYSIS_MODEL` env vars, per this item's own text that an evaluation
   result alone does not authorize a config change. **That separate deploy step was carried out the
   same day, 2026-08-28**: `.env` now sets `SERVICE_STRUCTURE_MODEL=gpt-5.6-luna` (effort unchanged at
   `medium`) and adds `ANALYSIS_MODEL=gpt-5.6-luna` / `ANALYSIS_REASONING_EFFORT=low`, which were
   previously unset and relying on the `gpt-5.6-terra`/low code default; `.env.example` comments were
   updated to match. Verified live with `config:show` before and after — note that command prints the
   OpenAI API key in plaintext on this config tree. **The six queue workers were restarted**
   (`queue.worker`, `-historic-ffmpeg`, `-historic-llm`, `-historic-orchestration`, `-historic-whisper`,
   `-video`): they read `.env` once at boot, so real processing would have silently continued on
   Sol/Terra without the restart, even though a one-off `artisan` command picks up the new values
   immediately. IC5 item 6 is therefore closed end to end — evaluated, ruled, adopted, deployed — and
   the definitive 470-identity corpus run below may proceed under `gpt-5.6-luna` for both call families.
...
| Run and approve the six-identity local-Whisper video calibration and gated cheaper-model comparison in IC5 item 6 | **Evaluator, inputs and first two sermon-analysis draws done 2026-08-27; the blocker is now human, not code.** Operator completes the six truth worksheets and the blinded sermon scoring (portable text-only pack: `calibration-review-pack/portable-20260827/`, blinding key held separately at `calibration-blinding-key-20260827.json`). Structure arms then run from `structure-eval-20260827/manifest.json`, whose expectations are empty by design until that truth exists. Sermon-analysis arms must **not** be re-run for the transcript recovery — banked sermon transcripts are byte-unchanged and still hash-match both reports. Operator records the accepted model/effort per call family, worker width and measured 470-identity cost | Definitive video dispatch |
| ~~**Build a producer for `crockenhill.historic-runtime.v1` runtime evidence**~~ | **DONE 2026-08-27**, `historic-import:capture-runtime-evidence`. Observes or refuses, nothing defaulted: commit, package-lock, migration manifest, database, hashed ffmpeg/ffprobe binaries, prompt and algorithm hashes, queues, CPU/memory, and provider connectivity from one live unbilled models call reused for the outbound probe and clock. Refuses a mock provider, a local sermon disk (a local filesystem cannot evidence encryption at rest from inside the container) and a non-TLS endpoint. Three inputs are required rather than observed — image digest, and how bucket encryption at rest/in transit was confirmed — and their notes are hashed into the artifact so the claim carries provenance. Output is validated through the real preflight before retention. Verified end to end against a live runtime. | ~~Any production operation~~ — closed |
| ~~**Measure whether whole-service priming causes transcript loops corpus-wide**~~ | **DONE 2026-08-28, zero spend.** `historic-import:measure-transcription-priming` re-transcribed all six banked calibration recordings whole-file under both prompts, three draws each (36 passes, local Whisper), replaying the detector plus a sub-threshold repetition census over every draw. Windowed or single-draw designs were rejected: the four windows recovery examined were selected *because the primed run failed there*, and the banked corpus already holds four primed passes over byte-identical `2024-01-14` audio at 130/2157/2776/3651 cues, so within-arm variance spans any effect a single pair could show. **Result: the hypothesis is partly confirmed and its implied remedy is refuted.** Priming roughly doubles looping (mean repeated-cue share 30.2% vs 15.3%; more on 5 of 6 identities and 11 of 18 draw-matched pairs, sign test p≈0.11 — directional, not established at n=6). But it does **not** increase what the pipeline flags: detector windows are 35 primed vs **40 unprimed**, flagged seconds 16,821 vs 16,066, so dropping the prompt would mark *more* material unobservable. Unprimed loops are equally service-shaped ("Lord, we thank you for all of us." ×251, "All praise to him who sang his name." ×123) because the audio genuinely is a service — the prompt amplifies a tendency the model already has over sung and non-speech audio rather than creating it. `transcription.prompts.full_service` is therefore **left unchanged**. Two consequences do carry forward, below. Report: `priming-measurement-20260828/report.json`, 36 transcripts colocated, banked inputs hash-unchanged, `processing_records_created: false` | ~~Definitive video dispatch~~ — closed |
| ~~**Collapse is not reproducible between passes — one pass cannot evidence completeness**~~ | **RETRACTED AND RESOLVED 2026-08-28, same day it was written.** The row measured the raw transcription step and stated its conclusion at the wrong layer: slice 5 persists what `ServiceTranscriptRecovery` returns, not what Whisper first emits. Replaying the real recovery over all 36 measured draws (zero spend, banked audio + banked transcripts, `priming-recovery-20260828/report.json`) shows recovery does the heavy lifting — the deterministic `2024-01-14` collapse goes **134 -> 1406 cues**, and `2022-06-05`'s 129-cue collapse fully heals to 2335 cues with zero residual windows. **No dispatch re-draw logic is warranted, and none was built.** What survives recovery is not draw-dependent noise but identity-determined audio: see the row below | ~~Definitive video dispatch~~ — closed |
| ~~**Improve local-Whisper transcription accuracy on the failing windows**~~ | **INVESTIGATED AND CLOSED 2026-08-28, zero spend. Every lever tested came back negative; do not re-tread these.** Benchmark was `2024-12-22`'s first 10 minutes, which production renders as 20 segments of `"Thank you."` and nothing else. (1) **Decode-confidence filtering is impossible**: fabricated segments reach 0.998 mean word probability while genuine speech falls to 0.569, and `no_speech_prob` is 0.0000 for both — the model is confidently wrong, which is why the text-repetition detector exists. (2) `--suppress-nst` inert; `-bs 5` beam search **worse** (40 fabrications vs 21); `-sns -bs 5` worse. (3) **No speech is hiding under the noise floor**: the region is -46 dB mean / -26 dB peak, and neither `volume=20dB` nor `loudnorm` to -17.5 dB surfaced one real word. IC5's "the audio itself is the limit" is confirmed. (4) **VAD works but crashes whisper-server 1.8.4**: Silero correctly reports `Final speech segments after filtering: 0`, then the server dies in language auto-detect on 0 ms of audio (`unknown language id -2`) and does not recover — a hard crash, reproduced with `language=en` pinned. **Never enable `--vad` on the server, on either version**: retested on 1.9.2 after upgrading, with `language=en` pinned per-request *and* `-l en` at server start, and it crashes identically — an unattended 470-identity run would lose the server on precisely the dead-air recordings VAD is meant to fix. This is an upstream bug, not a configuration error. **But VAD works perfectly as a standalone validator** — see the row below. **whisper-cpp upgraded 1.8.4 -> 1.9.2 on 2026-08-28 and verified output-neutral**: three of four reference clips are byte-identical, and the fourth is the known non-deterministic `2024-01-14` collapse region whose 1.9.2 spread (38/124/234 segments) brackets the 1.8.4 draw (81), so there is no version effect to re-validate. Note the machine carries two Homebrew prefixes — the arm64 `/opt/homebrew` one runs the server; the x86 `/usr/local` one is unused and is what produced an early false 'no Metal' reading. Performance was separately measured and is **not** worth tuning: already Metal-accelerated at 28x realtime, GPU-saturated (threads inert, two servers only 1.14x), and the corpus is ~20 h of unattended transcription | Nothing; accuracy floor is understood |
| ~~**Would a larger model transcribe the failing windows better?**~~ | **TESTED AND REJECTED 2026-08-28: full `large-v3` is decisively WORSE than `large-v3-turbo` here, and must not be adopted.** Four clips, replicated, same server build and prompt (repeated-cue share): on the **clean** control turbo holds 0.0% across draws while large-v3 collapses to **85.0/86.1%**; on the healthy `2026-07-26` speech clip turbo is a stable 6.0-6.8% against large-v3's **82.0%** on every draw; on `2024-01-14`'s collapse region turbo is 82.8-91.1% against large-v3's **98.9-100%**. Neither model reads dead air (both 100% repeated). Turbo is also ~3x faster, so there is no speed/accuracy trade here at all — turbo simply wins. **The operator's prior was right and for the right reason**: this is hallucination filling non-speech, not a capacity limit, so decoder depth (32 layers vs turbo's 4) buys nothing. **The genuinely new finding is a model x prompt interaction.** large-v3 *unprimed* is flawless on the clean control (0.0%, 146 segments, stable) and only collapses when given `transcription.prompts.full_service` — an 85-point swing from the prompt alone, where the same prompt costs turbo nothing on that clip. Unprimed large-v3 is still not adoptable: on the loop cases it is more variable than turbo, not less (loopy 32.4/95.0/99.5%; loopy2 1.7/5.3/24.7%), and for an unattended 470-identity run turbo's *stability* matters more than a good best-case draw. This also independently re-confirms leaving the production prompt alone: its cost is model-specific and turbo tolerates it. `ggml-large-v3.bin` (3.1 GB) left in `~/models/whisper.cpp/` and safe to delete | Nothing; model choice settled |
| **Scope the unobservable-window song veto using VAD, not audio level** | Coding agent; recommended, not yet built. The level-based version of this refinement was rejected the same day (row below) because peak level cannot separate a flagged window from ordinary audio. **VAD separates it cleanly and needs no tuned threshold.** The standalone `whisper-vad-speech-segments` binary (shipped with whisper-cpp, Silero model at `~/models/whisper.cpp/ggml-silero-v5.1.2.bin`) reports **0** speech segments on `2024-12-22`'s failing carol window against 59 on `2024-01-14`'s collapse region and 142/172 on ordinary speech. It runs at ~250x realtime (2.4 s for 600 s), so it is free at corpus scale. Proposed rule: when recovery flags an unobservable window, run VAD over that window; **zero speech segments means the window contains no speech and therefore cannot conceal a sung item, so it must not veto song corroboration**; any speech present keeps today's veto unchanged. This is the distinction the whole-recording veto currently cannot draw, and it is the right one: `2024-12-22` would flip from permanently-neutral to corroborating, while `2024-01-14` correctly stays vetoed because there really is speech there we failed to read. Note this is a **truth-rule change** on `ChurchServiceProjector::livestreamProvesSongDimensions()` and needs the usual focused coverage plus a decision before building | Slice 5 song-corroboration yield |
| ~~**Scope the unobservable-window song veto to silent windows**~~ | **PROPOSED AND REJECTED ON EVIDENCE 2026-08-28.** `ChurchServiceProjector::livestreamProvesSongDimensions()` vetoes song corroboration for a *whole* recording on any unobservable window, so 21.5 minutes of dead air at the head of `2024-12-22` discards 721 correctly-transcribed segments of the carol service that follows. The proposed refinement — a silent window cannot conceal a song, so it should not veto — **does not survive measurement**. Peak level relative to each recording's own peak (a scale-free rule, chosen precisely because the six identities share no noise floor) fails to separate: the 18 distinct flagged windows sit 0.0-3.4 dB below their recording peak while 14 matched control regions sit 0.0-15.5 dB below — total overlap, and the windows are if anything *louder* than random controls. Cause: the detector sets window bounds from the repeated-cue run, so a window spans the quiet air where hallucination starts **plus** the loud real audio it runs into. The window as flagged is therefore not dead air and "silence cannot hide a song" does not apply to it. A finer sub-window split might still work but is speculative; the current whole-recording veto stays | Nothing; the veto is correct as written |
| **Residual unobservable windows are irreducible audio, not a transcription defect — and they cost song corroboration** | Coding agent + operator judgement, before or alongside slice 5. After recovery, 18 of 36 draws still carry >=1 unobservable window, which makes the whole recording neutral for song membership/count/order. That rate is **not random across draws**: `2020-04-26` is clean 6/6 and `2022-06-05`/`2025-02-02` 5/6, while `2024-01-14` and `2024-12-22` are neutral in **all six** draws under both prompts, every residual window reading `retranscription_failed` — the audio was examined and yields nothing. `2024-12-22`'s window is its first ~20 minutes (the carols), consistent with the isolated-retry result already recorded in item 6. Only `2026-07-26` (clean 2/6) would materially benefit from a re-draw. So the exposure to slice 5 is **under-delivery, not corruption**: a permanently-neutral identity withholds song evidence rather than asserting wrong evidence, which is exactly the "remain explicitly unresolved" case §0.1 slice 5 already provides for. **The open question is quantitative**: 2 of 6 calibration identities are permanently neutral, and if that rate carried to 470 it would leave ~155 unable to corroborate songs — against a slice whose purpose is settling 223 services that have no corroborating source at all. The calibration six were chosen for shape diversity and deliberately include `short_partial` and `fragmented` material, so they are **not** a random sample and very likely over-state the rate. Measure the real rate on the first bounded pass and re-decide before dispatching the remainder; do not extrapolate 2/6 to the corpus | Slice 5 value assessment, not slice 5 dispatch |
| **Fix the `tests/Unit/Services/` processing-log leak** | Coding agent. Some test there commits a `media_processing_logs` row outside its transaction, so three `ImportHistoricVideoBatchCommandTest` cases fail when the two are run together while passing in isolation. Pre-existing and unrelated to the transcript work — reproduced 2026-08-27 with none of that day's tests present — but it makes combined gate runs read red for the wrong reason | Nothing; gate hygiene |
