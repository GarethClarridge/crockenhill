...
|---|---|---|
| Try a targeted `--fresh-parse --item-key=2023-01-01` retry; if it reproduces the date-arithmetic miss, accept as a hold with that reason recorded | Coding agent, cheap single-source retry | Final Email/OpenLP RG-A |
| Retry `2015-12-13` (failed on a transient OpenAI rate limit during the 2026-08-24 full-corpus check, not a corpus defect) | Coding agent, plain re-run | Final Email/OpenLP RG-A |
| Rule on `2024-10-06`'s date mismatch (detected `2024-10-04`), same defect class as `2023-01-01` | Operator; likely bundle with the `2023-01-01` retry | Final Email/OpenLP RG-A |
