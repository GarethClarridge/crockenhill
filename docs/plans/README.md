...
|---|---|---|---|
| H0 | [Historic incremental convergence](HISTORIC-IMPORT-INCREMENTAL-CONVERGENCE-2026-08-14.md) | **Plan of record; reconciled 2026-08-25.** Email/OpenLP corpus settlement is complete: 553 approved Email sources, 539 identities, 20 operator-approved holds, all 614 OpenLP includes, and zero unstaged or unexplained identities. The 398-proposal/21-class convergence census remains red by design: 223 services await another source and 135 currently disagree. REV-D4 is met; this red final gate does not block staging video. IC2 command authority/resume gaps remain open. Historic-video corroboration grade is not yet propagated to the Livestream source revision, so no video calibration is authorised. No production mutation is authorised. | Close the command-binding/video-resume gaps and propagate the hash-bound historic corroboration grade end to end. Only `full` historic recordings may prove song membership/count/order. Then draft/freeze the 462-identity video corpus, calibrate, run it resumably and re-census before classifying the surviving residue. |
...
| Historic operator inputs: review video worksheet exceptions/freeze; recover current-era sources after remeasurement; decide only the genuinely irreducible residue after the post-video census; approve era releases and §8.4 policy | Only the historic programme; see the incremental convergence plan §0 and §10 |
