...
Reviewed 2026-07-02, per `docs/archived-plans/JULY-2026-SIMPLIFICATION-REVIEW-PLAN-2026-07-02.md`. Light session, as planned. No code changes; findings only.
...
- **`LegacyPlayDateSongUsageImporter` + `ImportLegacySongUsageCommand`** (~480 lines + 2 test files): backfills usage from a SQL dump whose default path (`prod-20260326.sql`) does not exist in the repo. `docs/archived-plans/SIMPLIFICATION-PLAN.md` (lines ~330–350) already carries the tasks: confirm the historic import is complete, delete the pair + tests, and drop the `play_date` table. **Status unchanged since 2026-06-03** — this review re-confirms the code is untouched and the dump absent. Nothing new to discover; it needs the maintainer decision, not more analysis.
