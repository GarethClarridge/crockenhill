...
2. **Richer podcast metadata becomes a template edit, not a project.** With one RSS template (F8) and the already-typed `PodcastFeedItemReadModel`, adding Podcast 2.0 tags is trivial: `<podcast:transcript>` (the read model already carries `transcriptUrl` — it is simply not emitted today), `<podcast:person>` for the preacher, and per-episode chapter marks once section timestamps are exposed. A combined "all sermons" feed or per-series feeds would be a new config entry + the same template.
