...
The admin form dutifully captures `meeting_date`/`is_recurring`/`frequency` (`app/Livewire/Forms/MeetingFormData.php`, including a `frequencyOptions()` picker). On the *public* side the only reader is the inline Schema.org `Schedule` markup in `meetings/show.blade.php:43-102`. But the fields are **not** admin-dead: `app/Livewire/Admin/Meetings/ListMeetings.php:98-114` selects `is_recurring`/`frequency`, exposes a `recurringFilter`, and sorts on `is_recurring`, and `resources/views/livewire/admin/meetings/list-meetings.blade.php:24,78-81` renders the filter control and the recurrence/frequency badges. So the church maintains two scheduling models — hand-entered recurrence rules and the actual Google Calendar — where the recurrence rules drive nothing a *visitor* sees except JSON-LD, but do drive an admin filter, sort, and display column.

**Direction:** delete the occurrence calculators, the four event accessors, and the unused scopes/helpers now (~150 lines, zero risk — these have no consumers). The recurrence *fields* are a separate, larger decision (needs-decision item 6.4): if they stay, they are read by both the `Schedule` JSON-LD and the admin listing/filter/form; if they go, the removal must also strip the `ListMeetings` select/filter/sort, the blade filter control and badges, and the `MeetingFormData` plumbing — not just the schema markup — or the admin meeting table breaks. Scope that decision across all those consumers, not the public JSON-LD alone.
...
| 6.4 | `Meeting` recurrence fields (`meeting_date`, `is_recurring`, `frequency`) + form plumbing, once 4.5's dead calculators are gone | Operators hand-maintain schedule data duplicating Google Calendar; consumers are the inline `Schedule` JSON-LD **and** the admin `ListMeetings` select/filter/sort + list-meetings badges + `MeetingFormData` picker | Medium — `Schedule` markup re-derived from `CalendarEvent` (or dropped); **admin listing filter/sort/badges and the form picker must be removed too, not just the JSON-LD**; admin form shrinks; migration to drop columns |
