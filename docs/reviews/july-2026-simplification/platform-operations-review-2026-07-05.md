...
- **Migrations** — 166 files in `database/migrations/`, `database/schema/mysql-schema.sql` (1,222 lines), the 11 test files that `require` migration files at run time, `scripts/check-schema-dump-current.sh`.
...
- **34 console commands, of which 18 (53%) are one-shot migrate/backfill/cutover/calibration tools** (~2,615 command lines + companion services + ~2,000 test lines). Only 6 commands are wired to anything automatic (4 scheduled, 2 in deploy). The backlog's PR 1 records that **7 one-time migration commands were already deleted once** — this is the second generation of the same accumulation.
...
### F1. The command inventory is over half residue; the live surface is 16 commands
...
**Operational, on-demand, recurring value (8):** `CreateApiToken`, `ProcessVideoCommand` (manual sermon creation from segments), `SyncSongsCommand`, `LinkSongsCommand`, `SyncSermonScriptureFilters`, `AuditSchemaGuardrailsCommand` (pre-flight for Warden-style constraint work), `AssessSermonVideoQualityCommand` (single-sermon mode is operational; its backfill mode is spent — sermons review), and `BootstrapSpeakerProfilesCommand` — despite its backfill-sounding name it is the documented remediation for the "no speaker profiles yet" state in the admin preacher form (`resources/views/livewire/admin/preachers/preacher-form.blade.php:103`), with `--preacher=` targeting for new preachers and `--model-version=` for embedding-model refreshes (media review parked it here; disposition: **keep as operational**, rename-consideration for Phase 9).
...
**Dead keys in live files:** `config/auth.php` defines an `api` guard (`driver => sanctum`) that nothing uses — every protected route uses `auth:sanctum` middleware, which does not read it; remove the block. But the *file* belongs in the drifted-behind bucket above, not the delete-today one: the project copy predates `passwords.users.throttle => 60` (absent key = throttling off, so deletion would newly rate-limit password-reset token generation on an app with live reset routes) and `password_timeout => 10800` — the same conscious-adoption call as `hashing.php`. `config/calendar.php` `uncategorized_slug` and `performance.cache_duration` are confirmed unread (public-site review's parked item — verified here).
...
2. **The blocker is bounded and splits cleanly in two.** **11 test files (~21 test methods)** `require` migration files at run time — consistent with the count recorded when `--prune` was last assessed. Three are dedicated corrective-migration test classes (`CorrectiveSchemaMigrationsTest`, `FormalizePagesAreaColumnMigrationTest`, `AddIsAdminToUsersTableIfMissingMigrationTest`) that protect migrations already run in production — delete the whole files with the prune; the dump *is* their encoded outcome. The other eight (`ChurchServiceSchemaTest`, `ChurchServiceItemSchemaTest`, `SongCatalogSchemaTest`, `SermonIdentityAuthoritySchemaTest`, `ReportingStatePromotionSchemaTest`, `DataIntegrity/SpeakerIntegrityTest`, `Warden/SongIntegrityTest`, `Warden/SermonScriptureFilterMigrationTest`) are live schema/integrity suites in which only the migration-requiring methods (typically `down()`/re-run idempotency checks) need deleting or rewriting against the live schema — the rest of each suite survives untouched. (`tests/Concerns/CreatesSlugViolatingSermons` mentions a migration only in a docblock; no action.)
...
**Direction:** run `schema:dump --prune`, clear the 11 affected test files per the split above (3 whole-file deletions, 8 method-level strips), and **update the drift gate in the same PR** — `scripts/check-schema-dump-current.sh` currently encodes the pre-squash policy: it iterates `database/migrations/*.php` with an unquoted, non-nullglob glob (an empty directory yields a phantom missing migration named `*` and a CI failure) and its fix message says `schema:dump (never --prune)`. It needs to tolerate an empty migrations directory and drop the "never --prune" instruction; the check itself remains valuable for post-squash migrations. 166 files → 0 (until the next migration lands). The add-then-revert pairs and the 62-file integrity churn collapse into the dump instead of being replayed forever.
...
1. **Squash → absorbing the agent fleet becomes free (F3).** A quarterly one-command re-squash means Warden-style autonomous integrity work no longer has a compounding cost; the operator can keep the nightly fleet without the migrations directory becoming an archaeology site. Fresh CI/local setup loads one 1,222-line dump instead of replaying 166 files, and the ~21 migration-re-run test methods stop running on every CI pass.
2. **Pruned commands → `artisan list` becomes an honest operations menu (F1).** Sixteen live commands, each of which does something an operator might actually want today. That is real onboarding and incident-response value: no more deciding mid-incident whether `sermons:migrate-storage` is safe to run. The retirement-trigger convention keeps it honest permanently.
...
|---|---|---|---|
| P1 | Eight unowned one-shot commands (F1 table: `ConvertJpgToWebp`, `ImportHistoricVideoBatch`, `ExtractVideoFrames`, `ImportOpenLpDirectory`, `ExportVisualMetrics`, `BackfillMediaProcessingIdentity`, `FixUploadDirectories`, `MeetingMigratePhotos`+service) + tests (~1,480 prod lines + tests) | Second-generation dead weight; CI runs their tests forever; `artisan list` noise | Per-command prod confirmation needed (§8 Q1); git history is the re-run path. `ExportVisualMetrics`/`ExtractVideoFrames` are additionally gated on Phase 2 heuristic retirement if kept for calibration |
| P2 | All 166 migration files + the 3 corrective-migration test files + the migration-requiring methods in 8 schema/integrity suites (~21 tests total) via `schema:dump --prune` | 62-file churn replayed on every fresh DB; add-then-revert pairs preserved forever; regrows ~100/year | Low: dump is current + CI-gated; prod `migrations` table already records all files. Costs: the method-level test strips need per-suite care (F3.2), and `git log` on a migration file needs the pre-prune history |
...
| P4 | `hashing.php` (adopting `HASH_VERIFY=true` + `rehash_on_login`), `auth.php` (adopting `passwords.users.throttle => 60` + `password_timeout`, after the P5 guard removal), and `debugbar.php` (env-only control, losing `hide_empty_tabs`) | Behavioral drift persists silently | Small, *positive* behavior changes — but they are behavior changes; needs a conscious yes. The auth throttle touches live password-reset flows (Dusk covers them) |
...
1. **Per-command production status (P1)** — for each of the eight: has it been run to completion in production, and is a re-run plausible? Specifically: (a) was the WebP conversion (`ConvertJpgToWebp`) finished? (b) is the historic video import done, or are more batches expected? (c) are more historic OpenLP archives expected (`service-tracking:import-openlp-services`), or is `sync-songs` the only live path now? (d) was the media-log identity backfill run? (e) were meeting photos migrated? (f) does server provisioning still rely on `upload:fix-directories`?
2. **Squash sign-off (P2)** — comfortable clearing the 11 migration-requiring test files (3 whole-file deletions, 8 method-level strips per F3.2) with the prune, and adopting a quarterly re-squash?
...
4. **`hashing.php`/`auth.php`/`debugbar.php` (P4)** — adopt current framework/package defaults as described? For `auth.php` specifically: is a 60-second throttle on password-reset token generation acceptable (it is the framework default the file currently suppresses by omission)?
