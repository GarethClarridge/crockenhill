...
## 2026-07-22 - Scoped Target Locking for Dynamic Row Inputs
**Learning:** When using dynamic inline inputs (like custom dropdown selects or dynamic action buttons) within table rows or attention lists, locking the whole table or causing global UI flickering degrades the user experience. Implementing highly scoped `wire:target` definitions matching the dynamic method signature and individual record ID (e.g., `wire:target="categorize({{ $event->id }})"`) ensures that only the specifically modified row is locked during asynchronous processing, keeping other list items fully responsive.
**Action:** Apply highly scoped `wire:target` and `wire:loading.class="pointer-events-none"` definitions targeting specific record IDs on list rows or dynamic action elements to restrict only the interacting row.
