...
## 2026-07-18 - Select-triggered Row Lock and Disabled Select UX
**Learning:** For list views that support inline modification via dropdown selectors (e.g. categorizing an event via a `<select>`), triggering a server action on change needs precise row and element-level locks. Locking the entire `<tr>` using `pointer-events-none` prevents keyboard and hover interactions on that row, but explicitly pairing it with `wire:loading.attr="disabled"` on the select element itself ensures the input cannot be manipulated or double-submitted during the network roundtrip.
**Action:** Always add `wire:loading.attr="disabled"` and a matching `wire:target` on inline dropdown/select elements that trigger Livewire actions, in addition to row-level locks.
