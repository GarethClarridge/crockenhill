...
## 2026-07-07 - Documenting Orchestration and Result-Aware Exceptions
**Learning:** In complex orchestrator services like `ProcessingRunOrchestrator`, it is critical to distinguish between methods that throw hard exceptions (e.g., `start()` throwing `InvalidArgumentException` on unrecognized profiles) and those that catch internal errors to return a structured `ProcessingResult` (e.g., `retry()`). Documentation should explicitly mention the "surgical retry" strategies—dispatching chains vs. restarting—to bridge the gap between high-level orchestration and low-level job offsets.
**Action:** When documenting orchestrators, specify which methods bubble exceptions to the caller and which encapsulate failures within DTOs. Always explain the "why" behind multi-path recovery logic to aid future maintenance.
