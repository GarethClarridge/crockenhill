...
class ThumbnailGenerationService
{
    ...
    public function __construct(VideoSegmentationService $videoService)
    {
        ...
        // Initialize FFmpeg with same configuration as VideoSegmentationService
        $this->ffmpeg = FFMpeg::create([
            'ffmpeg.binaries' => $this->config['ffmpeg']['path'],
            'ffprobe.binaries' => $this->config['ffmpeg']['probe_path'],
    ...
    public function createBrandedThumbnail(Sermon $sermon, string $baseFramePath): ?string
    {
        try {
            ...
            // Add brand overlay if configured
    ...
    private function addTextOverlays($image, Sermon $sermon): void
    {
        ...
        // Calculate responsive positioning based on image dimensions
        $imageWidth = $image->width();
        $imageHeight = $image->height();
        
        // Adjust positioning for different image sizes
        $titleX = $this->calculateResponsivePosition($posConfig['title_x'], $imageWidth, 1280);
        $titleY = $this->calculateResponsivePosition($posConfig['title_y'], $imageHeight, 720);
        $dateX = $this->calculateResponsivePosition($posConfig['date_x'], $imageWidth, 1280);
        $dateY = $this->calculateResponsivePosition($posConfig['date_y'], $imageHeight, 720);
    ...
    /**
     * Add brand overlay to thumbnail image with proper positioning to avoid text overlap
     * 
     * @param \Intervention\Image\Image $image The image to modify
     * @return void
     */
    private function addBrandOverlay($image): void
    {
        ...
        $brandPosition = $this->config['overlay']['brand_position'];
        $brandMargin = $this->config['overlay']['brand_margin'];
        ...
        try {
            ...
            // Scale brand overlay if it's too large for the image
            $maxBrandWidth = $image->width() * 0.25; // Max 25% of image width
            $maxBrandHeight = $image->height() * 0.25; // Max 25% of image height
            
            if ($brandOverlay->width() > $maxBrandWidth || $brandOverlay->height() > $maxBrandHeight) {
                $brandOverlay->resize($maxBrandWidth, $maxBrandHeight, function ($constraint) {
                    $constraint->aspectRatio();
                    $constraint->upsize();
                });
            }
            ...
            $position = $this->calculateBrandPosition($image, $brandOverlay, $brandPosition, $brandMargin);
            
            // Insert brand overlay at calculated position
            $image->insert($brandOverlay, 'top-left', $position[0], $position[1]);
    ...
    private function addTextWithBackground($image, string $text, int $x, int $y, int $fontSize, string $fontColor, array $bgConfig): void
    {
        try {
            ...
            $this->addTextBackground($image, $x, $y, $textBounds, $bgConfig);
            
            // Add main text with Oswald font
            $image->text($text, $x + $bgConfig['padding'], $y + $bgConfig['padding'], function($font) use ($fontSize, $fontColor, $fontPath) {
                $font->size($fontSize);
    ...
    /**
     * Calculate brand overlay position to avoid text overlap
     * 
     * @param \Intervention\Image\Image $image Main image
     * @param \Intervention\Image\Image $brandOverlay Brand overlay image
     * @param string $position Position string (e.g., 'bottom-right')
     * @param int $margin Margin from edges
     * @return array Position coordinates [x, y]
     */
    private function calculateBrandPosition($image, $brandOverlay, string $position, int $margin): array
    {
        $imageWidth = $image->width();
        $imageHeight = $image->height();
        $brandWidth = $brandOverlay->width();
        $brandHeight = $brandOverlay->height();
        
        // Calculate positions to avoid text overlap
        $textAreaHeight = $this->config['overlay']['positioning']['date_y'] + 100; // Approximate text area
        ...
        switch ($position) {
            ...
            case 'top-right':
                return [$imageWidth - $brandWidth - $margin, $margin];
                
            case 'bottom-left':
                return [$margin, $imageHeight - $brandHeight - $margin];
                
            case 'bottom-right':
                // Ensure brand doesn't overlap with text area
                $y = max($textAreaHeight + $margin, $imageHeight - $brandHeight - $margin);
                return [$imageWidth - $brandWidth - $margin, $y];
                
            case 'center':
                return [
                    ($imageWidth - $brandWidth) / 2,
                    ($imageHeight - $brandHeight) / 2
                ];
                
            default:
                // Default to bottom-right with text avoidance
                $y = max($textAreaHeight + $margin, $imageHeight - $brandHeight - $margin);
                return [$imageWidth - $brandWidth - $margin, $y];
        }
    }
    ...
    private function addTextBackground($image, int $x, int $y, array $textBounds, array $bgConfig): void
    {
        try {
            ...
            // Create background rectangle with white color and opacity
            $bgColor = $bgConfig['color'];
            $opacity = $bgConfig['opacity'];
            
            // Convert hex color to RGB
            $rgb = $this->hexToRgb($bgColor);
            $bgColorWithOpacity = sprintf('rgba(%d, %d, %d, %.2f)', $rgb[0], $rgb[1], $rgb[2], $opacity);
            
            // Draw rectangle background
            $image->rectangle($x, $y, $x + $bgWidth, $y + $bgHeight, function ($draw) use ($bgColorWithOpacity) {
                $draw->background($bgColorWithOpacity);
            });
     ...
     * 
     * @param int $basePosition Base position for reference resolution
     * @param int $currentDimension Current image dimension
     * @param int $referenceDimension Reference dimension from config
     * @return int Scaled position
     */
    private function calculateResponsivePosition(int $basePosition, int $currentDimension, int $referenceDimension): int
    {
        $scale = $currentDimension / $referenceDimension;
        return (int) ($basePosition * $scale);
    }
