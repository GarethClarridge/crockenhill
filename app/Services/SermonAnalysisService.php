...
class SermonAnalysisService implements SermonAnalysisInterface
{
    ...
    private function performAiAnalysis(string $transcript, array $existingSeries, string $processingId = 'unknown'): array
    {
        ...
        while ($attempt < $maxRetries) {
            ...
            try {
                ...
                $validatedData = $this->parseAiResponse($response, $transcript, $processingId);
    ...
    /**
     * Execute the OpenAI API request with targeted error handling.
     *
     * @throws Exception
     */
    private function executeAiRequest(string $prompt, string $model, string $processingId, int $attempt): mixed
    {
        try {
     ...
     * @throws Exception
     */
    private function parseAiResponse(mixed $response, string $transcript, string $processingId): array
    {
        ...
        return $this->validator->validateAndCleanAnalysisData($analysisData, $transcript);
    }
