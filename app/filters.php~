...
View::composer('layouts.sections.footer', function($view)
{
    ...
    $latest_morning_sermon = Sermon::where('service', 'morning')->orderBy('date', 'desc')->first();
    $latest_evening_sermon = Sermon::where('service', 'evening')->orderBy('date', 'desc')->first();
...
View::composer('layouts.sections.sermonsfragment', function($view)
{
    ...
    $serieses = Sermon::where('series', '!=', 'NULL')->select('series')->distinct()->get();
