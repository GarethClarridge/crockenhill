# GEMINI.md — Gemini Code Assist Addendum

**Read [AGENTS.md](AGENTS.md) first** — it holds the project overview, architecture, commands, conventions, and quality gates that apply to every coding agent on this repo. This file only carries the Boost-managed Laravel guidelines block; everything else lives in `AGENTS.md`.

The `<laravel-boost-guidelines>` block below is auto-regenerated by `vendor/bin/sail artisan boost:install`. Do not hand-edit it — re-run that command after major package upgrades.
