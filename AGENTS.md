...
The `<laravel-boost-guidelines>` block at the bottom is generated by `vendor/bin/sail artisan boost:install` — do not hand-edit it. Re-run that command after major package upgrades to refresh it.
...
- **ProcessingRouter** — Routes media uploads to the right processor based on type.
- **VideoProcessingService** — Video segmentation (livestreams) and direct processing (sermons).
- **SermonProcessingService** — Audio processing, transcription, AI analysis.
...
2. **Video** — Full extraction with video preservation → sermon record with video.
